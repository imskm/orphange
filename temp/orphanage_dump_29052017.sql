...
INSERT INTO `adopt` VALUES (1,'2017-05-14 12:32:49',1,2,1),(2,'2017-05-28 15:33:59',2,2,3);
...
--
-- Temporary table structure for view `adopt_child_V`
--

DROP TABLE IF EXISTS `adopt_child_V`;
/*!50001 DROP VIEW IF EXISTS `adopt_child_V`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8;
/*!50001 CREATE VIEW `adopt_child_V` AS SELECT 
 1 AS `ChId`,
 1 AS `FName`,
 1 AS `LName`,
 1 AS `Dob`,
...
INSERT INTO `appointment` VALUES (1,'2017-05-13 00:23:01','2017-05-12 00:23:01','You have not uploaded your documents of proof, so we can not verify your identity!',3,2,1,1),(2,'2017-05-29 06:00:00','2017-05-12 00:26:38',NULL,2,1,2,1),(3,'2017-05-28 15:32:08','2017-05-28 15:32:08',NULL,1,2,2,3);
...
CREATE TABLE `donation` (
  ...
  KEY `AdopId` (`AdopId`),
  KEY `OId` (`OId`),
  CONSTRAINT `donation_ibfk_1` FOREIGN KEY (`AdopId`) REFERENCES `adopter` (`AdopId`),
  CONSTRAINT `donation_ibfk_2` FOREIGN KEY (`OId`) REFERENCES `orphanage` (`OId`)
...
INSERT INTO `donation` VALUES (1,'2017-05-21 21:16:33',2000,NULL,'F5200','Just a simple donation test.',2,2,2),(2,'2017-05-21 21:19:07',3000,NULL,'FF900','Another donation test.',1,2,3),(3,'2017-05-22 19:58:57',4000,'Bank of India','E9F013','Happy donating.',2,1,1),(4,'2017-05-23 13:31:58',9000,'Punjab National Bank','dr123','',2,2,1);
...
--
-- Final view structure for view `adopt_child_V`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `adopt_child_V` AS select `g`.`ChId` AS `ChId`,`g`.`FName` AS `FName`,`g`.`LName` AS `LName`,`g`.`Dob` AS `Dob`,`g`.`Gender` AS `Gender`,`g`.`Age` AS `Age`,`g`.`Photo` AS `Photo`,`g`.`OId` AS `OId`,`a`.`AdoptId` AS `AdoptId` from (`godchild` `g` left join `adopt` `a` on((`a`.`ChId` = `g`.`ChId`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2017-05-29 16:28:32
